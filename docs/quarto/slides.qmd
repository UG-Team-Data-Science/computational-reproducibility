...
## A GitHub Codespace for the FDV book

1. Sign in [GitHub](https://github.com/login)
2. Open the `fdvcwcontainer` [branch](https://github.com/Venustiano/datavizclaus/tree/fdvcwcontainer) in GitHub
3. Click on `Code` -> `CodeSpaces` -> `+` (Create a codespace ...)
4. The creation of the `codespace` will take some time
5. When done, **Do not forget to stop the codespace**

::: aside
Once the codespace has been created, you can open `JupyterLab` instead of `VScode` by clicking on the dots `...` and `Open in JupyterLab`
:::
